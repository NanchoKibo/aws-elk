...
sudo tar xvf kibana-3.1.1.tar.gz -C /opt/kibana/kibana3 --strip-components=1
sudo rm -f kibana-3.1.1.tar.gz

sudo curl -O https://download.elasticsearch.org/kibana/kibana/kibana-4.0.0-beta3.tar.gz
sudo tar xvf kibana-4.0.0-beta3.tar.gz -C /opt/kibana/kibana4 --strip-components=1
